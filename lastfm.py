...
store_file='C:/Kaige_Research/Graph_based_recommendation_system/Data/lastfm_small_data_set/'
results_folder='C:/Kaige_Research/Graph_based_recommendation_system/Result/lastfm/'

newpath = results_folder+'results_'+str(timeRun)+'/'
if not os.path.exists(newpath):
    os.makedirs(newpath)
################### Original data
user_json=np.load(store_file+'lastfm_popular_user_json.npy').item()#237
article_features_array=np.load(store_file+'top_1000_users_article_features.npy')
article_features_array=Normalizer().fit_transform(article_features_array)
popular_article_list=np.load(store_file+'lastfm_popular_item_list.npy')
...
plt.plot(cd_cum_reward, label='SCLUB-CD',color='r', marker='X', linewidth=1, markevery=0.1, markersize=8)
plt.legend(loc='best', fontsize=12)
plt.yticks(rotation=90)
